...
Purpose: Users need to see a consent banner on first visit, make an informed accept/reject choice, and be able to change their mind later via the footer. This satisfies BNRR-01 through BNRR-04, BNRR-06, BNRR-07, and all A11Y requirements.
...
- **Hydration guard:** Return `null` if `!isHydrated || hasConsented !== null` — MUST be checked before any hooks that depend on DOM (the early return pattern). Note: React hooks must be called unconditionally, so use state/ref initialization before the guard but keep effects conditional internally, OR structure the component so that conditional rendering is handled by a wrapper. The simplest correct approach: call all hooks first, then conditionally render `null` at the JSX return level based on a computed `shouldShow` variable. Do NOT put the early return before hook calls.
...
- **Escape key effect:** Add a `keydown` listener on `document` that calls `setConsent(false)` when `Escape` is pressed. Research recommended dismiss-without-recording, but the planning context says "Escape key: dismiss without recording consent (leaves hasConsented = null, banner returns)". However, the research ALSO has the code using `setConsent(false)`. Use the simpler approach: Escape dismisses without setting consent. Create a local `dismissed` state (useState, default false). When Escape is pressed, set `dismissed = true`. Include `dismissed` in the `shouldShow` condition: `isHydrated && hasConsented === null && !dismissed`. This way the banner hides but `hasConsented` stays null, so the banner returns on next page load. Clean up the keydown listener on unmount.
